...
int main()
{
	...
	fd_w1 = open("3_1.fifo", O_WRONLY);
	fd_r1 = open("1_3.fifo", O_RDONLY);
	fd_r2 = open("2_3.fifo", O_RDONLY);
	fd_w2 = open("3_2.fifo", O_WRONLY);
	char buf[1024] = "";
	char buf2[1024] = "";
	...
	if(fork() > 0)
	{
		...
		{
			fprintf(fp1, "From cl_3:%s", buf);
			fprintf(fp2, "From cl_3:%s", buf);
			fflush(fp1);
			fflush(fp2);
		}
		close(fd_w1);
